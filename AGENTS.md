...
- All themes with a dashboard topbar must expose `#top-color-mode-button` using `.icon-top-brightness`; shared `initColorModeToggle()` manages `html[data-color-mode]` and persists the mode in `localStorage` under `minpanel.colorMode`, while each theme stylesheet owns its dark palette overrides.
